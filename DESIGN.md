...
- **Typography:** Bar and panel links both Figtree 500 at `1rem`, and they stay there. `0.875rem` reads too small on this site, so nav type is not a lever for the width budget below. The wordmark is **Figtree 800 at `1.875rem`, `tracking-tight`** — a logotype, not display type, which is why it does not take Instrument Serif.
- **States:** Rest is `text-white/90` (14.6:1). Hover fills to `--color-white` over a `white/8` wash. Active page takes `--color-white` plus a `2px` brass underline inset to match the link's own padding. The underline is the one place in this system where brass exceeds a hairline, and it is not a border: the Hairline Rule governs side-stripes on cards and callouts, not a wayfinding mark under a link. Never pure `#fff` — the Tinted Neutral Rule holds here too. **The bar runs a bright resting state on purpose**, which leaves only `1.23:1` between rest and active: the brass underline, not the text colour, is what marks the current page, and the `white/8` wash, not the text colour, is what marks hover. Strengthen the underline before reaching for the text colour.
